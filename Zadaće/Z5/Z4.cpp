// TP 2021/2022: Zadaæa 5, Zadatak 4
#include <algorithm>
#include <cmath>
#include <fstream>
#include <iostream>
...
class PolimorfniSpremnik{
    Spremnik *pokazivaci;
    void DaLiJeOkej() const{
        if(!pokazivaci) throw std::logic_error("Nespecificiran spremnik");
    }
    public:
    PolimorfniSpremnik() {pokazivaci=nullptr;}
    ~PolimorfniSpremnik() {delete pokazivaci;}
    PolimorfniSpremnik(const Spremnik &s){ pokazivaci=s.DajKopiju();}
    PolimorfniSpremnik(const PolimorfniSpremnik &ps){
        if(!ps.pokazivaci) pokazivaci=nullptr;
        else pokazivaci=ps.pokazivaci->DajKopiju();
    }
    PolimorfniSpremnik(PolimorfniSpremnik &&ps){
        pokazivaci=ps.pokazivaci;
        ps.pokazivaci=nullptr;
        //pokazivaci=std::move(ps.pokazivaci);
    }
    PolimorfniSpremnik &operator=(const PolimorfniSpremnik &ps){
        Spremnik *pom=nullptr;
        if(ps.pokazivaci!=nullptr){
            pom=ps.pokazivaci->DajKopiju();
        }
        delete pokazivaci;
        pokazivaci=pom;
        return *this;
    ...
    double DajTezinu() const{
        DaLiJeOkej();
        return pokazivaci->DajTezinu();
    }
    double DajUkupnuTezinu() const{
        DaLiJeOkej();
        return pokazivaci->DajUkupnuTezinu();
    }
    void Ispisi() const{
        DaLiJeOkej();
        pokazivaci->Ispisi();
